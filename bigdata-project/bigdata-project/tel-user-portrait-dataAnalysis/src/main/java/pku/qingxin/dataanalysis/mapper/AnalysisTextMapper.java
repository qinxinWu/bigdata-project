...
public class AnalysisTextMapper extends TableMapper<Text,Text>{
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        //呼叫者用户-年
        context.write(new Text(call1 + "-" + year),new Text(duration));
        //呼叫者用户-月
        context.write(new Text(call1 + "-" + month),new Text(duration));
        //呼叫者用户-日
        context.write(new Text(call1 + "-" + day),new Text(duration));

        //被呼叫者用户-年
        context.write(new Text(call2 + "-" + year),new Text(duration));
        //被呼叫者用户-月
        context.write(new Text(call2 + "-" + month),new Text(duration));
        //被呼叫者用户-日
        context.write(new Text(call2 + "-" + day),new Text(duration));
    }
}
